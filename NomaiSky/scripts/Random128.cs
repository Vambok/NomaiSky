...
public class Random128
{
    struct Xorshift32(uint seed) {
        ...
        readonly uint firstState = seed != 0 ? seed : 0xCAFEBABE;
        ...
        readonly uint FixedUInt(uint id) {
            uint fState = firstState + id;
            fState ^= fState << 13;
            fState ^= fState >> 17;
            fState ^= fState << 5;
            return fState;
        ...
        public readonly int Range(int minInclusive, int maxExclusive, uint id) { return (minInclusive == maxExclusive ? minInclusive : (int)(FixedUInt(id) % (uint)(maxExclusive - minInclusive)) + minInclusive); }
        public readonly float Range(float minInclusive, float maxExclusive, uint id) { return minInclusive + ((maxExclusive - minInclusive) * FixedUInt(id) / uint.MaxValue); }
        public readonly bool RandomBool(uint id) { return (FixedUInt(id) & 1) == 0; }
    }
    ...
    public void Restart() { cursor = 3; }
    uint Hash(string seed) {
        uint res = 0; int i = 0;
        foreach(char c in seed) {
            res += (uint)((int)c switch { < 32 => 0, < 48 => 1, < 58 => c - 46, < 65 => 1, < 91 => c - 53, < 97 => 1, < 123 => c - 59, _ => 0 }) << (i % 5 * 6);
            i++;
        }
        return res;
    }
    public int Range(int minInclusive, int maxExclusive, string seed, bool start = false) { if(start) cursor = 3; return NextStream().Range(minInclusive, maxExclusive, Hash(seed)); }
    public float Range(float minInclusive, float maxExclusive, string seed, bool start = false) { if(start) cursor = 3; return NextStream().Range(minInclusive, maxExclusive, Hash(seed)); }
    public bool RandomBool(string seed, bool start = false) { if(start) cursor = 3; return NextStream().RandomBool(Hash(seed)); }
    public int[] GeneratePermutations(string seed = "", bool start = false) {
        ...
        if(seed == "") {
            ...
            uint hash = Hash(seed);
            for(int i = 255;i > 0;i--) {
                int j = NextStream().Range(0, i + 1, hash);
                (baseValues[i], baseValues[j]) = (baseValues[j], baseValues[i]);
            }
        }
